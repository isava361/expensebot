...
// - /cancel для отмены текущего мастера, чтобы не «залипать» в состоянии.
// - Кнопки switch_inline_query для рассылки /join в любой чат.
//
// ENV:
...
func (a *app) cb(b *tgb.Bot, ctx *ext.Context) error {
	...
	case strings.HasPrefix(data, "explist|"):
		...
		if len(parts) >= 3 && stringsHasPrefix(parts[2], "p:") {
	...
	case strings.HasPrefix(data, "expdel|"):
		...
		if len(parts) >= 4 && stringsHasPrefix(parts[2], "gid:") && stringsHasPrefix(parts[3], "p:") {
	...
	case strings.HasPrefix(data, "pay|"):
		...
		parts := strings.Split(data, "|")
		if len(parts) == 4 && stringsHasPrefix(parts[1], "gid:") && stringsHasPrefix(parts[2], "to:") && stringsHasPrefix(parts[3], "amt:") {
...
func (a *app) sendGroupDetailsEdit(b *tgb.Bot, ctx *ext.Context, gid int64) error {
	...
	// Кнопки (приглашение + список трат + удалить + назад)
	rows := [][]tgb.InlineKeyboardButton{
		{{Text: "Открыть бота по ссылке", Url: link}},
		{{Text: "Отправить /join в чат…", SwitchInlineQuery: fmt.Sprintf("/join %s", code)}},
		{{Text: "Вставить /join сюда", SwitchInlineQueryCurrentChat: fmt.Sprintf("/join %s", code)}},

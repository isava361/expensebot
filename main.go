...
// - Кнопка «Поделиться ссылкой» через t.me/share/url с текстом "/join <код>" (без @username).
//
// ENV:
...
func (a *app) cb(b *tgb.Bot, ctx *ext.Context) error {
	...
	case strings.HasPrefix(data, "invsel|"):
		...
		_, _ = ctx.CallbackQuery.Answer(b, &tgb.AnswerCallbackQueryOpts{Text: "Откроется выбор чата"})
		return nil
...
func (a *app) sendGroupDetailsEdit(b *tgb.Bot, ctx *ext.Context, gid int64) error {
	...
	// Кнопка открытия системного шэринга Telegram с текстом "/join <код>".
	share := fmt.Sprintf("https://t.me/share/url?text=%s", url.QueryEscape("/join "+code))
	rows := [][]tgb.InlineKeyboardButton{
